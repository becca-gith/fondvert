<?php
/**
 * Template part : Page "Plainte" – version statique
 * Fonds Vert du Togo
 *
		...
		<div class="plainte-grid">
			...
			<div class="plainte-info">
				...
				<ul class="plainte-info__list">
					<li><i class="fas fa-check-circle"></i> Toute plainte est traitée avec confidentialité et rigueur.</li>
					<li><i class="fas fa-check-circle"></i> Vous pouvez déposer une plainte de manière anonyme.</li>
					<li><i class="fas fa-check-circle"></i> Un accusé de réception vous sera envoyé sous 48 heures (si vous laissez vos coordonnées).</li>
					<li><i class="fas fa-check-circle"></i> Une enquête indépendante est menée par notre comité d'éthique.</li>
					<li><i class="fas fa-check-circle"></i> Aucune mesure de rétorsion ne sera prise contre le plaignant de bonne foi.</li>
				</ul>
			...
			<div class="plainte-form">
				...
				<form id="plainte-form" method="post" enctype="multipart/form-data">
					...
					<div id="identite-fields">
						...
						<div class="form-row">
							<div class="form-group">
								...
								<input type="email" id="email" name="email" placeholder="exemple@domaine.tg">
							</div>
							<div class="form-group">
				...
				<div id="plainte-success" style="display:none;">
					...
					<button class="plainte-reset" onclick="resetPlainteForm()">Déposer une autre plainte</button>
				</div>
			</div>
...
<style>
/* ============================================================
   PAGE PLAINTE – CHARTE FONDS VERT TOGO
   ============================================================ */
:root {
	--vert-fvt:        #0a6e3e;
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		const successDiv = document.getElementById('plainte-success');
		const anonymeCheck = document.getElementById('anonyme');
		const identiteFields = document.getElementById('identite-fields');
		const requiredFields = identiteFields.querySelectorAll('input[required]');
		...
		form.addEventListener('submit', function(e) {
			...
			const submitBtn = form.querySelector('.plainte-submit');
			...
			setTimeout(function() {
				...
				submitBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Envoyer la plainte';
			}, 2000);
		});
	});
	...
	function resetPlainteForm() {
		...
		document.getElementById('plainte-form').style.display = 'block';
		document.getElementById('plainte-success').style.display = 'none';
		// Réinitialiser l'affichage de l'identité
		const anonymeCheck = document.getElementById('anonyme');
		const identiteFields = document.getElementById('identite-fields');
		if (anonymeCheck.checked) {
			anonymeCheck.checked = false;
			identiteFields.style.display = 'block';
			const requiredFields = identiteFields.querySelectorAll('input');
			requiredFields.forEach(function(field) {
				field.setAttribute('required', 'required');
			});
		}
		document.querySelector('.plainte-form').scrollIntoView({ behavior: 'smooth' });
	}
</script>