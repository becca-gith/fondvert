<?php
/**
 * Template part : Page "Guichet" générique – version améliorée
 * Récupère le contenu, les chiffres clés et les projets depuis la page (slug guichet-*)
 *
 * @package TogoGreenFund
 */

$page = get_queried_object();
if ( ! $page || strpos( $page->post_name, 'guichet-' ) !== 0 ) {
...
// Préfixe dynamique
$prefix = str_replace( 'guichet-', '', $page->post_name );
...
if ( strpos( $page->post_name, 'foret' ) !== false || strpos( $page->post_name, 'biodiversite' ) !== false ) {
    $icon = 'fa-tree';
    $color = '#2d6a4f';
}
if ( strpos( $page->post_name, 'eau' ) !== false ) {
    $icon = 'fa-water';
    $color = '#0077b6';
}
if ( strpos( $page->post_name, 'energie' ) !== false ) {
    $icon = 'fa-solar-panel';
    $color = '#f4a261';
}
if ( strpos( $page->post_name, 'agriculture' ) !== false ) {
    $icon = 'fa-seedling';
    $color = '#0a6e3e';
}
?>
<style>
/* ============================================================
   PAGE GUICHET – STYLE AMÉLIORÉ
   ============================================================ */
:root {
    --guichet-color: <?php echo $color; ?>;
    --guichet-light: <?php echo $color; ?>22;
    --guichet-dark: <?php echo $color; ?>cc;
    --guichet-shadow: 0 8px 32px rgba(0,0,0,0.08);
}
...
.guichet-header::before {
    ...
    width: 80%;
    height: 200%;
    background: radial-gradient(circle at 70% 30%, var(--guichet-light) 0%, transparent 70%);
    pointer-events: none;
    opacity: 0.6;
}
.guichet-header::after {
    content: '';
    position: absolute;
    bottom: 0;
    left: 0;
    right: 0;
    height: 4px;
    background: linear-gradient(90deg, var(--guichet-color) 0 33%, #ffce00 33% 66%, #d21034 66% 100%);
}
.breadcrumb-wrapper {
    margin-bottom: 20px;
}
...
.guichet-header__badge {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    padding: 6px 20px;
    border-radius: 30px;
    background: var(--guichet-light);
    color: var(--guichet-color);
    font-family: 'Kumbh Sans', sans-serif;
    font-size: 13px;
    font-weight: 700;
...
.title-underline {
    width: 80px;
    height: 4px;
    background: linear-gradient(90deg, var(--guichet-color), #ffce00);
    margin: 16px auto 0;
    border-radius: 4px;
}
.guichet-header__sub {
...
.guichet-intro__card {
    ...
    background: #fff;
    border-radius: 24px;
    box-shadow: var(--guichet-shadow);
    overflow: hidden;
    border: 1px solid #e7f0ea;
    transition: transform 0.3s, box-shadow 0.3s;
}
.guichet-intro__card:hover {
    transform: translateY(-4px);
    box-shadow: 0 16px 40px rgba(0,0,0,0.10);
}
.guichet-intro__texte {
    padding: 40px 36px;
}
.guichet-intro__texte .first-paragraph {
    font-family: 'Kumbh Sans', sans-serif;
    font-size: 1.12rem;
    font-weight: 500;
    color: #063d24;
...
.guichet-intro__texte .first-paragraph::first-letter {
    font-size: 3.8rem;
    font-weight: 800;
    font-family: 'Playfair Display', serif;
    color: var(--guichet-color);
...
.guichet-intro__texte p:last-child {
    margin-bottom: 0;
}
.guichet-intro__image {
    overflow: hidden;
}
.guichet-intro__image img {
    width: 100%;
    height: 100%;
    object-fit: cover;
    transition: transform 0.6s;
}
.guichet-intro__card:hover .guichet-intro__image img {
...
.chiffre-item:hover {
    ...
    border-color: var(--guichet-color);
}
...
<!-- ============================================================
     EN‑TÊTE
     ============================================================ -->
<section class="guichet-header" style="--guichet-color: <?php echo $color; ?>;">
    <div class="container">
        ...
        <span class="guichet-header__badge"><i class="fas <?php echo $icon; ?>"></i> Togo Green Fund</span>
        <h1><?php echo esc_html( $page_title ); ?></h1>
        <div class="title-underline"></div>
        <p class="guichet-header__sub"><?php echo esc_html( get_post_meta( $page_id, '_guichet_subtitle', true ) ?: 'Découvrez nos actions dans ce secteur.' ); ?></p>
    </div>
</section>
...
<!-- ============================================================
     CONTENU
     ============================================================ -->
<section class="guichet-content">
    <div class="container">
        <!-- Introduction -->
        <div class="guichet-intro">
            <div class="guichet-intro__card">
                <div class="guichet-intro__texte">
                    <?php echo $page_content ?: '<p class="first-paragraph">Ajoutez le contenu de cette page depuis l\'éditeur.</p>'; ?>
                </div>
                <div class="guichet-intro__image">
                    <?php
                    $image_url = get_the_post_thumbnail_url( $page_id, 'large' );
                    if ( empty( $image_url ) ) {
                        $image_url = 'https://images.unsplash.com/photo-1625246333195-78d9c38ad449?w=600&h=400&fit=crop';
                    }
                    ?>
        ...
        <div class="guichet-section">
            <h2 class="guichet-section__title"><i class="fas fa-rocket"></i> Projets emblématiques</h2>
            <div class="guichet-projets">
...
<!-- ============================================================
     CTA
     ============================================================ -->
<section class="guichet-cta">
    <div class="container">
        <h2>Vous souhaitez agir dans ce secteur ?</h2>
        <div class="cta-divider"></div>
        <p>Rejoignez le Togo Green Fund pour contribuer à un avenir durable.</p>
